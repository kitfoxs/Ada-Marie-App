...
enum AdaMarieChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AdaMarieChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AdaMarieChatTheme.onboardingAssistantBubble"),
...
enum AdaMariePlatformImageFactory {
    static func image(_ image: AdaMariePlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
