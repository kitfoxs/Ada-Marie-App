...
@MainActor
struct AdaMarieChatComposer: View {
    @Bindable var viewModel: AdaMarieChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AdaMarieChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AdaMarieChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AdaMariePendingAttachment.id)
                { (att: AdaMariePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AdaMariePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(AdaMarieChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AdaMarieChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AdaMarieChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AdaMarieChatTheme.subtleCard)
        .clipShape(Capsule())
    }
