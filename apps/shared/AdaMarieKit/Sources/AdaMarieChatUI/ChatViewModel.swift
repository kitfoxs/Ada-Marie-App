...
private let chatUILogger = Logger(subsystem: "ai.adamarie", category: "AdaMarieChatUI")

@MainActor
@Observable
public final class AdaMarieChatViewModel {
    public private(set) var messages: [AdaMarieChatMessage] = []
    ...
    public var attachments: [AdaMariePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AdaMarieChatPendingToolCall] = []
    public private(set) var sessions: [AdaMarieChatSessionEntry] = []
    private let transport: any AdaMarieChatTransport
    ...
    private var pendingToolCallsById: [String: AdaMarieChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AdaMarieChatTransport) {
    ...
    public var sessionChoices: [AdaMarieChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [AdaMarieChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [AdaMarieChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AdaMariePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AdaMarieChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AdaMarieChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [AdaMarieChatMessage]) -> [AdaMarieChatMessage] {
        var result: [AdaMarieChatMessage] = []
    ...
    private static func dedupeKey(for message: AdaMarieChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AdaMarieChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AdaMarieChatAttachmentPayload in
            AdaMarieChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AdaMarieChatSessionEntry {
        AdaMarieChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AdaMarieChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AdaMarieChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AdaMarieAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AdaMarieChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AdaMariePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
