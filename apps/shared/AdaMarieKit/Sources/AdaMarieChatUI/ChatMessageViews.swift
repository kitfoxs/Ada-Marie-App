...
struct ChatMessageBubble: View {
    let message: AdaMarieChatMessage
    let style: AdaMarieChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AdaMarieChatTheme.userText : AdaMarieChatTheme.assistantText
    ...
    private var inlineAttachments: [AdaMarieChatMessageContent] {
    ...
    private var toolCalls: [AdaMarieChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AdaMarieChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AdaMarieChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AdaMarieChatTheme.onboardingAssistantBubble
        }
        return AdaMarieChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AdaMarieChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AdaMarieChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AdaMarieChatTheme.userText : AdaMarieChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AdaMarieChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AdaMarieChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AdaMarieChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AdaMarieChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AdaMarieChatTheme.assistantText)
            }
        }
    }
}
