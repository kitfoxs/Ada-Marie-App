...
import Foundation
import Testing
@testable import AdaMarieChatUI
...
private final class TestChatTransport: @unchecked Sendable, AdaMarieChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AdaMarieChatHistoryPayload]
    private let sessionsResponses: [AdaMarieChatSessionsListResponse]

    private let stream: AsyncStream<AdaMarieChatTransportEvent>
    private let continuation: AsyncStream<AdaMarieChatTransportEvent>.Continuation

    init(
        historyResponses: [AdaMarieChatHistoryPayload],
        sessionsResponses: [AdaMarieChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AdaMarieChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AdaMarieChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AdaMarieChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [AdaMarieChatAttachmentPayload]) async throws -> AdaMarieChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AdaMarieChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AdaMarieChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AdaMarieChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AdaMarieChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AdaMarieChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AdaMarieChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AdaMarieChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AdaMarieChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AdaMarieChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AdaMarieChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AdaMarieChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
