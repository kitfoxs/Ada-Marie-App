...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AdaMarieContactsSearchParams) async throws -> AdaMarieContactsSearchPayload {
        ...
        return AdaMarieContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AdaMarieContactsAddParams) async throws -> AdaMarieContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AdaMarieContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AdaMarieContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AdaMarieContactPayload {
        AdaMarieContactPayload(
