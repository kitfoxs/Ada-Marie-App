...
final class RemindersService: RemindersServicing {
    func list(params: AdaMarieRemindersListParams) async throws -> AdaMarieRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AdaMarieReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AdaMarieReminderPayload(
        ...
        return AdaMarieRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AdaMarieRemindersAddParams) async throws -> AdaMarieRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AdaMarieReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AdaMarieRemindersAddPayload(reminder: payload)
    }
