...
final class MotionService: MotionServicing {
    func activities(params: AdaMarieMotionActivityParams) async throws -> AdaMarieMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AdaMarieMotionActivityEntry], Error>) in
        ...
        return AdaMarieMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AdaMariePedometerParams) async throws -> AdaMariePedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<AdaMariePedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AdaMariePedometerPayload(
