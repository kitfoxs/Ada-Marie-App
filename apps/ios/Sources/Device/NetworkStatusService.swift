...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> AdaMarieNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> AdaMarieNetworkStatusPayload {
        let status: AdaMarieNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> AdaMarieNetworkStatusPayload {
        AdaMarieNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
