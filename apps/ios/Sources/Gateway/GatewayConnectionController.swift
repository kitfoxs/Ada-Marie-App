...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [AdaMarieCapability.canvas.rawValue, AdaMarieCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(AdaMarieCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(AdaMarieCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = AdaMarieLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(AdaMarieCapability.location.rawValue) }

        caps.append(AdaMarieCapability.device.rawValue)
        caps.append(AdaMarieCapability.photos.rawValue)
        caps.append(AdaMarieCapability.contacts.rawValue)
        caps.append(AdaMarieCapability.calendar.rawValue)
        caps.append(AdaMarieCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(AdaMarieCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            AdaMarieScreenCommand.record.rawValue,
            AdaMarieSystemCommand.notify.rawValue,
            AdaMarieChatCommand.push.rawValue,
        ...
        if caps.contains(AdaMarieCapability.camera.rawValue) {
            commands.append(AdaMarieCameraCommand.list.rawValue)
            commands.append(AdaMarieCameraCommand.snap.rawValue)
            commands.append(AdaMarieCameraCommand.clip.rawValue)
        }
        if caps.contains(AdaMarieCapability.location.rawValue) {
            commands.append(AdaMarieLocationCommand.get.rawValue)
        }
        if caps.contains(AdaMarieCapability.device.rawValue) {
            commands.append(AdaMarieDeviceCommand.status.rawValue)
            commands.append(AdaMarieDeviceCommand.info.rawValue)
        }
        if caps.contains(AdaMarieCapability.photos.rawValue) {
            commands.append(AdaMariePhotosCommand.latest.rawValue)
        }
        if caps.contains(AdaMarieCapability.contacts.rawValue) {
            commands.append(AdaMarieContactsCommand.search.rawValue)
            commands.append(AdaMarieContactsCommand.add.rawValue)
        }
        if caps.contains(AdaMarieCapability.calendar.rawValue) {
            commands.append(AdaMarieCalendarCommand.events.rawValue)
            commands.append(AdaMarieCalendarCommand.add.rawValue)
        }
        if caps.contains(AdaMarieCapability.reminders.rawValue) {
            commands.append(AdaMarieRemindersCommand.list.rawValue)
            commands.append(AdaMarieRemindersCommand.add.rawValue)
        }
        if caps.contains(AdaMarieCapability.motion.rawValue) {
            commands.append(AdaMarieMotionCommand.activity.rawValue)
            commands.append(AdaMarieMotionCommand.pedometer.rawValue)
        }
