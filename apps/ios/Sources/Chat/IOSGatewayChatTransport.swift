...
struct IOSGatewayChatTransport: AdaMarieChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AdaMarieChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AdaMarieChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AdaMarieChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AdaMarieChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AdaMarieChatAttachmentPayload]) async throws -> AdaMarieChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AdaMarieChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(AdaMarieChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(AdaMarieGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AdaMarieGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AdaMarieChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AdaMarieAgentEventPayload.self)
                        {
