...
This is an **alpha** iOS app that connects to an AdaMarie Gateway as a `role: node`.
...
3. On iOS: AdaMarie → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project AdaMarie.xcodeproj -scheme AdaMarie -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AdaMarieKit` contains the shared transport/types used by the iOS app.
