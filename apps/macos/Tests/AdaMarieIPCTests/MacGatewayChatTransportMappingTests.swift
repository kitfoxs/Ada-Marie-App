...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AdaMarieProtocol.AnyCodable(["ok": AdaMarieProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AdaMarieProtocol.AnyCodable(["ok": AdaMarieProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AdaMarieProtocol.AnyCodable([
            "runId": AdaMarieProtocol.AnyCodable("run-1"),
            "sessionKey": AdaMarieProtocol.AnyCodable("main"),
            "state": AdaMarieProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AdaMarieProtocol.AnyCodable(["a": AdaMarieProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
