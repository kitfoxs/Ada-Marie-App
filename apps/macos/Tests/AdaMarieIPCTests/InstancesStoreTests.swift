...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AdaMarieProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AdaMarieProtocol.AnyCodable] = [
            "presence": .init([AdaMarieProtocol.AnyCodable(entry)]),
        ]
        let payload = AdaMarieProtocol.AnyCodable(payloadMap)
