...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["adamarie"]
        let text = "hey adamarie"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey adamarie do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["adamarie"], minPostTriggerGap: 0.3)
