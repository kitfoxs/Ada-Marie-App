...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (AdaMarie token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: AdaMarieOAuthStore.AnthropicOAuthStatus = AdaMarieOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.adamarie", category: "anthropic-oauth")
...
enum AdaMarieOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let adamarieOAuthDirEnv = "OPENCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "AdaMarie OAuth token file not found"
            case .unreadableFile: "AdaMarie OAuth token file not readable"
            case .invalidJSON: "AdaMarie OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in AdaMarie OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "AdaMarie OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.adamarieOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".adamarie", isDirectory: true)
