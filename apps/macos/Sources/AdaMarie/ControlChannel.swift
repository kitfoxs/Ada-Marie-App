...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AdaMarieProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.adamarie", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AdaMarieKit.AnyCodable]()) {
                $0[$1.key] = AdaMarieKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AdaMarieProtocol.AnyCodable?) -> [String: AdaMarieProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AdaMarieProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AdaMarieKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AdaMarieProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("adamarie.control.heartbeat")
    static let controlAgentEvent = Notification.Name("adamarie.control.agent")
}
