...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.adamarie", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = AdaMarieConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.adamarieExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "adamarie CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "adamarie@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "adamarie@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "adamarie@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "adamarie@\(target)"])
            }

        statusHandler("Installing adamarie@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed adamarie@\(target)")
        } else {
