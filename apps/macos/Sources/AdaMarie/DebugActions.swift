...
enum DebugActions {
    private static let verboseDefaultsKey = "adamarie.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "adamarie.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AdaMariePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AdaMarie", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AdaMariePaths.configURL
        guard
