...
private let deepLinkLogger = Logger(subsystem: "ai.adamarie", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "AdaMarie is paused", message: "Unpause AdaMarie to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run AdaMarie agent?", message: body) else { return }
        }
