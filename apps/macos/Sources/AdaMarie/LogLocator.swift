...
enum LogLocator {
    private static var logDir: URL {
        ...
        let preferred = URL(fileURLWithPath: "/tmp/adamarie")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("adamarie-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("adamarie-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/adamarie/ (rolling or stdout), or nil if none exist.
