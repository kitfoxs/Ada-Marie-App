...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.adamarie", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AdaMarieSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AdaMarieSessionPreviewEntry,
    ...
    private static func cache(payload: AdaMarieSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AdaMarieChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AdaMarieKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AdaMarieKit.AnyCodable]) -> [AdaMarieChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AdaMarieChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AdaMarieChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AdaMarieChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AdaMarieChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AdaMarieChatMessage) -> String? {
