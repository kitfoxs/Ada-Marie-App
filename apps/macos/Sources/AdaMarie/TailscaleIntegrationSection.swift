...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.adamarie/adamarie.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.adamarie/adamarie.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.adamarie/adamarie.json. Restart the gateway to apply."
        }
