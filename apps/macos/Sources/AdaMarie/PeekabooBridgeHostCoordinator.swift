...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.adamarie", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AdaMariePeekabooBridgeServices?
    private static var adamarieSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("AdaMarie", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AdaMariePeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.adamarieSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.adamarieSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AdaMariePeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.adamarie.peekaboo")
