...
enum AdaMarieLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AdaMarieOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AdaMarieFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AdaMarieLogging.bootstrapIfNeeded()
        let label = AdaMarieLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AdaMarieOSLogHandler: LogHandler {
...
struct AdaMarieFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AdaMarieLogging.parseLabel(self.label)
