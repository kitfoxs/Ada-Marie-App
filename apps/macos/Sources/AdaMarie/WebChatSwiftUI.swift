...
private let webChatSwiftLogger = Logger(subsystem: "ai.adamarie", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AdaMarieChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AdaMarieChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AdaMarieChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AdaMarieChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AdaMarieGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    AdaMarieAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AdaMarieChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AdaMarieChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AdaMarieChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AdaMarieChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AdaMarieChatView>) -> NSViewController
    {
