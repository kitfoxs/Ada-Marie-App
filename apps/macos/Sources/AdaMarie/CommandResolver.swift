...
enum CommandResolver {
    private static let projectRootDefaultsKey = "adamarie.gatewayProjectRootPath"
    private static let helperName = "adamarie"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let adamarieEntry = root.appendingPathComponent("adamarie.mjs").path
        if FileManager().isReadableFile(atPath: adamarieEntry) { return adamarieEntry }
        let binEntry = root.appendingPathComponent("bin/adamarie.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/adamarie")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let adamariePaths = self.adamarieManagedPaths(home: home)
        if !adamariePaths.isEmpty {
            extras.insert(contentsOf: adamariePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + adamariePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func adamarieManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".adamarie"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func adamarieExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAdaMarieExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("adamarie.mjs").path,
            root.appendingPathComponent("bin/adamarie.js").path,
        ]
    ...
    static func hasAnyAdaMarieInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.adamarieExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func adamarieNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let adamariePath = self.projectAdaMarieExecutable(projectRoot: root) {
                return [adamariePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "adamarie", subcommand] + extraArgs
            }
            if let adamariePath = self.adamarieExecutable(searchPaths: searchPaths) {
                return [adamariePath, subcommand] + extraArgs
            }

            let missingEntry = """
            adamarie entrypoint missing (looked for dist/index.js or adamarie.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real adamarie CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/adamarie"
        ...
        \(cliSection)
        \(projectSection)
        if command -v adamarie >/dev/null 2>&1; then
          CLI="$(command -v adamarie)"
          adamarie \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/adamarie.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/adamarie.mjs"
            node "$PRJ/adamarie.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/adamarie.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/adamarie.js"
            node "$PRJ/bin/adamarie.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent adamarie"
          pnpm --silent adamarie \(quotedArgs);
        else
          echo "adamarie CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AdaMarieConfigFile.loadDict()
